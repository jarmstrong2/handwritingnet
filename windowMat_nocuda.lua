...
function Window:updateOutput(input)
    ...
    local alphas_t = torch.exp(hat_alphas_t)
    local betas_t = torch.exp(hat_betas_t)
    local kappas_t = self.kappas_t_1 + torch.exp(hat_kappas_t)--:div(25)
    
    local sampleSize = (#alphas_t)[1]
    
    local u_vector = torch.linspace(1, self.cu_size, self.cu_size)
    ...
    local output = torch.bmm(phi_t, cu_resized):squeeze(2)
...
function Window:updateGradInput(input, gradOutput)
    ...
    local alphas_t = torch.exp(hat_alphas_t)
    local betas_t = torch.exp(hat_betas_t)
    local kappas_t = self.kappas_t_1 + torch.exp(hat_kappas_t)
    ...
    local calc = torch.cmul(gradOutput_expanded, cu_resized)
